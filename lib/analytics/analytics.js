...
exports.logRequest = function (trackingCode, req) {
    ...
    var createParams = function (trackingCode, req) {
        ...
        params.tid = trackingCode; // Tracking ID - TODO: read from config
        params.uip = getClientAddress(req); // IP address of user
        params.ua = req.headers['user-agent'];
        params.dh = req.headers['host']; // hostname, for example 'www.mapa-kriminality.cz'
        params.dp = req.url; // path, for example '/api/times '
        return params;
    };
