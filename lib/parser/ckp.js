...
exports.Client = function () {
    ...
    var loadPage = function loadPage(url, callback, errorCallback) {
        request(url, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                callback(body);
            } else {
                errorCallback("Failed to load CKP.cz website.");
            }
        })
    };
    ...
    self.search = function (spz, date, callback, errorCallback) {
        ...
        loadPage(url, function (body) {
            ...
            if(rows.length <= 1) { // only header, no data
                callback(formatResults([], url));
            } else if(rows.length == 2) { // standard, one header and one data row
                callback(formatResults([parseRow($, rows[1], url)], url));
                ...
                callback(formatResults(results, url));
            }
        }, errorCallback);
    };
